...
| GET | `/triage` | `poweruser` | `ALLOW_DIAGNOSTICS=true` | The same run walked from the symptom: one playbook per symptom, every step answered by the checks that already ran. Adds no check and reads no API. |
