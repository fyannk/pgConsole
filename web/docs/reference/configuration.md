...
| `INSTANCE_STATUS_ENABLED` | `true` | Reads each instance manager's own status report (`/pg/status` on port `8000` of every instance pod) on a sweep: archiver state, pending restarts, replication slots, the manager's version — the report the operator itself reads and `kubectl cnpg status` prints. Strict boolean. On by default like the metrics sweep, and reaching pod IPs the same way, so the NetworkPolicy must allow that egress. The report is read, never written; from CloudNativePG 1.30 the status port serves TLS and its writing paths need the operator's certificate, while this path does not. TLS is tried first and plain HTTP when TLS is refused. The certificate is the cluster's own, signed by a CA that lives in a Secret the console never reads, so it is **not verified**: the trust rests on the pod IP the API server reported, exactly as the metrics sweep's does. |
