...
| `ALLOW_DIAGNOSTICS` | `false` | Enables the diagnostics screen, which correlates facts the other screens already carry into findings. Strict boolean; needs no Role — every check reads the published snapshots, plus the API server's public `/version` report used for version-pinned checks. |
...
| `LOG_STREAM_ENABLED` | `ALLOW_DIAGNOSTICS && ALLOW_LOGS` | Follows the member containers' logs continuously so the diagnostics matcher can analyse each line as it arrives. It keeps no tail: a line that matches no rule is analysed and dropped. A line that **does** match is kept verbatim, bounded per rule per container, and quoted as that finding's evidence — so a PostgreSQL error record carrying statement text is held in the console's memory and shown on the diagnostics screen. The default is derived rather than fixed: following is on where the screen that reads it is on and the tail is permitted, and off where either is missing, since a fixed `true` would refuse to start under `ALLOW_LOGS=false`. Set it explicitly to override either way; `true` with `ALLOW_LOGS=false` is still refused as conflicting intent. |
...
| `LOG_MATCH_MAX_AGE` | `6h` | A diagnostics log observation expires this long after its last matching line, `1m`–`168h`, so a finding cannot outlive its relevance. A recurring failure renews its window on every match. |
...
| `INSTANCE_STATUS_ENABLED` | `true` | Reads each instance manager's own status report (`/pg/status` on port `8000` of every instance pod) on a sweep: archiver state, pending restarts, replication slots, the manager's version — the report the operator itself reads and `kubectl cnpg status` prints. Strict boolean. On by default like the metrics sweep, and reaching pod IPs the same way, so the NetworkPolicy must allow that egress. The report is read, never written; from CloudNativePG 1.30 the status port serves TLS and its writing paths need the operator's certificate, while this path does not. TLS is tried first and plain HTTP when TLS is refused. The certificate is the cluster's own server certificate, issued for its Services; without `INSTANCE_STATUS_CA_FILE` it is checked for naming this cluster's read-write Service and nothing more, so the endpoint's identity rests on the pod IP the API server reported, exactly as the metrics sweep's does. |
| `INSTANCE_STATUS_CA_FILE` | *empty* | Absolute path of a PEM file holding the cluster CA — the `<cluster>-ca` Secret's `ca.crt`, mounted by the deployer as a volume rather than read through the API. With it, the status port's certificate chain is verified against the CA and must name the cluster's read-write Service: full verification. Requires `INSTANCE_STATUS_ENABLED=true`; an unreadable file fails before listen. |
| `INSTANCE_STATUS_INTERVAL` | `15s` | Sweep period, 5s–5m. The instance manager recomputes the report on every request, which is why the sweep is slower than the metrics one. |
