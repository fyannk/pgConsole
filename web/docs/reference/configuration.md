...
| `LOG_STREAM_ENABLED` | `ALLOW_DIAGNOSTICS && ALLOW_LOGS` | Follows the member containers' logs continuously so the diagnostics matcher can analyse each line as it arrives. Retains no log text of its own — only the lines that matched a rule, bounded per rule per container, and each is quoted verbatim as a finding's evidence. The default is derived rather than fixed: following is on where the screen that reads it is on and the tail is permitted, and off where either is missing, since a fixed `true` would refuse to start under `ALLOW_LOGS=false`. Set it explicitly to override either way; `true` with `ALLOW_LOGS=false` is still refused as conflicting intent. |
